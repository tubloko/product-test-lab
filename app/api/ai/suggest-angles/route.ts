...
import { SUGGEST_ANGLES_SYSTEM, buildSuggestAnglesUser } from '@/lib/claude/prompts';
import {
  SuggestAnglesInputSchema,
...
const ENDPOINT = 'suggest-angles' as const;
...
export async function POST(req: NextRequest) {
  ...
  const parsed = SuggestAnglesInputSchema.safeParse(await req.json().catch(() => null));
  if (!parsed.success) {
  ...
  try {
    const response = await getAnthropic().messages.create({
      ...
      system: SUGGEST_ANGLES_SYSTEM,
      messages: [{ role: 'user', content: buildSuggestAnglesUser(input) }],
    });
    const inputTokens = response.usage?.input_tokens ?? 0;
    const outputTokens = response.usage?.output_tokens ?? 0;
    const text = extractText(response);
    const output = parseAndValidate(text, SuggestAnglesOutputSchema);
