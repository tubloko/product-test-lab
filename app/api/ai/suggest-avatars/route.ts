...
import { SUGGEST_AVATARS_SYSTEM, buildSuggestAvatarsUser } from '@/lib/claude/prompts';
import {
  SuggestAvatarsInputSchema,
...
const ENDPOINT = 'suggest-avatars' as const;
...
export async function POST(req: NextRequest) {
  ...
  const parsed = SuggestAvatarsInputSchema.safeParse(await req.json().catch(() => null));
  if (!parsed.success) {
  ...
  try {
    const response = await getAnthropic().messages.create({
      ...
      system: SUGGEST_AVATARS_SYSTEM,
      messages: [{ role: 'user', content: buildSuggestAvatarsUser(input) }],
    });
    const inputTokens = response.usage?.input_tokens ?? 0;
    const outputTokens = response.usage?.output_tokens ?? 0;
    const text = extractText(response);
    const output = parseAndValidate(text, SuggestAvatarsOutputSchema);
