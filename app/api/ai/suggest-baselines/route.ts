...
import { SUGGEST_BASELINES_SYSTEM, buildSuggestBaselinesUser } from '@/lib/claude/prompts';
import {
  SuggestBaselinesInputSchema,
...
const ENDPOINT = 'suggest-baselines' as const;
...
export async function POST(req: NextRequest) {
  ...
  const parsed = SuggestBaselinesInputSchema.safeParse(await req.json().catch(() => null));
  if (!parsed.success) {
  ...
  try {
    const response = await getAnthropic().messages.create({
      ...
      system: SUGGEST_BASELINES_SYSTEM,
      messages: [{ role: 'user', content: buildSuggestBaselinesUser(input) }],
    });
    const inputTokens = response.usage?.input_tokens ?? 0;
    const outputTokens = response.usage?.output_tokens ?? 0;
    const text = extractText(response);
    const output = parseAndValidate(text, SuggestBaselinesOutputSchema);
