...
import { V2_SUGGESTIONS_SYSTEM, buildV2SuggestionsUser } from '@/lib/claude/prompts';
import {
  V2SuggestionsInputSchema,
...
const ENDPOINT = 'v2-suggestions' as const;
...
export async function POST(req: NextRequest) {
  ...
  const parsed = V2SuggestionsInputSchema.safeParse(await req.json().catch(() => null));
  if (!parsed.success) {
  ...
  const hypothesisId = input.hypothesis.id;
  ...
  try {
    const response = await getAnthropic().messages.create({
      ...
      system: V2_SUGGESTIONS_SYSTEM,
      messages: [{ role: 'user', content: buildV2SuggestionsUser(input) }],
    });
    const inputTokens = response.usage?.input_tokens ?? 0;
    const outputTokens = response.usage?.output_tokens ?? 0;
    const text = extractText(response);
    const output = parseAndValidate(text, V2SuggestionsOutputSchema);
    ...
    console.error(`[ai/${ENDPOINT}] generation failed`, { uid, hypothesisId, name, message, isTimeout });
