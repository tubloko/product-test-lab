...
import { VIABILITY_SUMMARY_SYSTEM, buildViabilitySummaryUser } from '@/lib/claude/prompts';
import {
  ViabilitySummaryInputSchema,
...
const ENDPOINT = 'viability-summary' as const;
...
export async function POST(req: NextRequest) {
  ...
  const parsed = ViabilitySummaryInputSchema.safeParse(await req.json().catch(() => null));
  if (!parsed.success) {
  ...
  try {
    const response = await getAnthropic().messages.create({
      ...
      system: VIABILITY_SUMMARY_SYSTEM,
      messages: [{ role: 'user', content: buildViabilitySummaryUser(input) }],
    });
    const inputTokens = response.usage?.input_tokens ?? 0;
    const outputTokens = response.usage?.output_tokens ?? 0;
    const text = extractText(response);
    const output = parseAndValidate(text, ViabilitySummaryOutputSchema);
